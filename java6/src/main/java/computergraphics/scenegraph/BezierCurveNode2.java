 ...
 */
public class BezierCurveNode2 extends LeafNode {
	private VertexBufferObject vbo = new VertexBufferObject();
	private BezierCurve bcurve;
	private double h = 0.0001;

	public BezierCurveNode2(BezierCurve bc) {
		this(bc, 0.01);
	}

	public BezierCurveNode2(BezierCurve bc, double resolution) {
	...
	public void comupteTangents() {
		...
		for (double i = 0; i <= 1; i += 0.2) {
			...
			renderVertices1.add(new RenderVertex(v, new Vector(0, 1, 0), new Vector(0, 0, 0, 1)));
		}
